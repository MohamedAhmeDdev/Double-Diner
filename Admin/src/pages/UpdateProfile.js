...
function UpdateProfile() {
  ...
  const validatePassword = (value) => {
    // If field is cleared, reset invalid flag
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      // Keep local token synchronization current
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      // Wipe the sensitive fields on success
      setPassword('');
      setConfirmPassword('');
        ...
        <form onSubmit={handleUpdateProfile} className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-10 border-b border-gray-200">
          <div className="md:col-span-1 space-y-1">
            ...
            <p className="text-sm text-gray-500 leading-relaxed max-w-xs">
              Change your main identifying markers and email addresses used for notification flows.
            </p>
          </div>
